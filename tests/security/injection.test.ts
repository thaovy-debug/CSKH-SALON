  ...
  describe("Error Response Safety", () => {
    it("should not leak database details in error responses", async () => {
      mockPrisma.conversation.findMany.mockRejectedValue(
        new Error(
          "PrismaClientKnownRequestError: Connection refused to database linhkienled1000_production"
        )
      );
      ...
      expect(JSON.stringify(data)).not.toContain("linhkienled1000_production");
